...
number = 115
folder_name = "MULTI_OBJECTIVE_115-115"
...
# Lista de valores posibles para las propiedades
values = ["NONE", "LOW", "MEDIUM", "HIGH", "VERYHIGH"]

# Construir el payload completo con valores aleatorios para las propiedades
payload = {
    "new_config_devices": new_config_devices,
    "available_devices": {device: [] for device in device_types},
    "properties": {key: random.choice(values) for key in ["security", "usability", "connectivity", "sustainability"]}
}
...
excel_filename = 'api_response_data_multi_objective.xlsx'
...
try:
    ...
    if not os.path.exists(excel_filename):
        ...
        sheet.title = 'API Response Data Multi Objective'
